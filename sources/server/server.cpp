...
void Server::Run() {
    ...
    while ((client_socket_ = accept(sock_fd_, reinterpret_cast<struct sockaddr *>(&addr_), &addr_len))) {
        ...
        std::cout << m_time() << "[SERVER] Accepted new connection from client with an " << inet_ntoa(addr_.sin_addr)
